...
void QuickItemPainter::paintQuickRectangle(QQuickItem *_item)
{
    ...
    if (border_width > 0 && !(border_width == 1 && border_color == QColor(Qt::black))) { // FIXME this is wrong, but there is no other clean way to detect if a border is valid or not
        _painter->setPen(QPen(border_color, border_width));
    } else {
        _painter->setPen(Qt::NoPen);
    }

    if (radius > 0) {
        _painter->drawRoundedRect(rect, radius, radius);
    } else {
...
void QuickItemPainter::paintQuickText(QQuickItem *_item)
{
    ...
    switch (textFormat) {
        case Qt::PlainText: { // Text.PlainText
            ...
            _painter->drawText(QRect(rect.x(), rect.y(), rect.width() + 5, rect.height()), text, textOption);
        }
        break;
        ...
        case 4: { // Text.StyledText
            ...
            textLayout.draw(_painter, rect.topLeft());
        }
        break;
        ...
        case Qt::RichText: { // Text.RichText
            ...
            _painter->translate(rect.topLeft());
            _painter->setRenderHint(QPainter::Antialiasing, true);
            layout->draw(_painter, context);
        }
        break;
    }
}
...
void QuickItemPainter::paintQuickImage(QQuickItem *_item)
{
    ...
    _painter->drawImage(rect, image, sourceRect);
}
...
void QuickItemPainter::paintItem(QQuickItem *_item)
{
    ...
    if (_item->flags().testFlag(QQuickItem::ItemHasContents)) {
        ...
        if (_item->clip()) {
            ...
            _painter->setClipRect(_item->clipRect());
        }
        ...
        if (inherits(_item->metaObject(), "QQuickRectangle")) {
            ...
            _painter->drawImage(rect.x(), rect.y(), image, rect.x(), rect.y(), rect.width(), rect.height());
        }
...
void QuickItemPainter::setDebugVerbose(bool _debugVerbose)
{
    _debugVerbose = _debugVerbose;
}
