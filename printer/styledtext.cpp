...
public:
    ...
    StyledTextPrivate(const QString &t, QTextLayout &l,
...
const QChar StyledTextPrivate::lessThan(QLatin1Char('<'));
const QChar StyledTextPrivate::greaterThan(QLatin1Char('>'));
const QChar StyledTextPrivate::equals(QLatin1Char('='));
const QChar StyledTextPrivate::singleQuote(QLatin1Char('\''));
const QChar StyledTextPrivate::doubleQuote(QLatin1Char('\"'));
const QChar StyledTextPrivate::slash(QLatin1Char('/'));
const QChar StyledTextPrivate::ampersand(QLatin1Char('&'));
const QChar StyledTextPrivate::bullet(0x2022);
const QChar StyledTextPrivate::disc(0x25e6);
const QChar StyledTextPrivate::square(0x25a1);
const QChar StyledTextPrivate::lineFeed(QLatin1Char('\n'));
const QChar StyledTextPrivate::space(QLatin1Char(' '));

StyledText::StyledText(const QString &string, QTextLayout &layout,
                       QList<StyledTextImgTag *> &imgTags,
                       const QUrl &baseUrl,
                       QQmlContext *context,
                       bool preloadImages,
...
void StyledText::parse(const QString &string, QTextLayout &layout,
                       QList<StyledTextImgTag *> &imgTags,
                       const QUrl &baseUrl,
                       QQmlContext *context,
                       bool preloadImages,
...
void StyledTextPrivate::appendText(const QString &textIn, int start, int length, QString &textOut)
{
    if (prependSpace) {
...
//
void StyledTextPrivate::setFontSize(int size, QTextCharFormat &format)
{
...
bool StyledTextPrivate::parseTag(const QChar *&ch, const QString &textIn, QString &textOut, QTextCharFormat &format)
{
    skipSpace(ch);
...
bool StyledTextPrivate::parseCloseTag(const QChar *&ch, const QString &textIn, QString &textOut)
{
    skipSpace(ch);
    int tagStart = ch - textIn.constData();
    int tagLength = 0;
...
void StyledTextPrivate::parseEntity(const QChar *&ch, const QString &textIn, QString &textOut)
{
...
bool StyledTextPrivate::parseFontAttributes(const QChar *&ch, const QString &textIn, QTextCharFormat &format)
{
    bool valid = false;
...
bool StyledTextPrivate::parseOrderedListAttributes(const QChar *&ch, const QString &textIn)
{
    bool valid = false;
    List listItem;
    listItem.level = 0;
...
bool StyledTextPrivate::parseUnorderedListAttributes(const QChar *&ch, const QString &textIn)
{
    bool valid = false;
    List listItem;
    listItem.level = 0;
...
bool StyledTextPrivate::parseAnchorAttributes(const QChar *&ch, const QString &textIn, QTextCharFormat &format)
{
    bool valid = false;
    QPair<QStringRef, QStringRef> attr;
...
void StyledTextPrivate::parseImageAttributes(const QChar *&ch, const QString &textIn, QString &textOut)
{
    qreal imgWidth = 0.0;

    if (!updateImagePositions) {
        StyledTextImgTag *image = new StyledTextImgTag;
        ...
        do {
            ...
            if (attr.first == QLatin1String("src")) {
                ...
                if (attr.second.toString() == QLatin1String("top")) {
                    image->align = StyledTextImgTag::Top;
                } else if (attr.second.toString() == QLatin1String("middle")) {
                    image->align = StyledTextImgTag::Middle;
                }
            }
        } while (!ch->isNull() && !attr.first.isEmpty());
        ...
        if (preloadImages && !image->size.isValid()) {
            ...
            if (url.isLocalFile()) {
                image->pix = QImage(url.toLocalFile());
        ...
        StyledTextImgTag *image = imgTags->value(nbImages);
        image->position = textOut.length() + 1;
        imgWidth = image->size.width();
        QPair<QStringRef, QStringRef> attr;
...
QPair<QStringRef, QStringRef> StyledTextPrivate::parseAttribute(const QChar *&ch, const QString &textIn)
{
    skipSpace(ch);
...
QStringRef StyledTextPrivate::parseValue(const QChar *&ch, const QString &textIn)
{
...
QString StyledTextPrivate::toAlpha(int value, bool upper)
{
...
QString StyledTextPrivate::toRoman(int value, bool upper)
{
